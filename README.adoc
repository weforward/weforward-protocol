...
|微服务名
|微服务的唯一标识
|小写字母+下划线+数字+点。如用户微服务：zuoche_user、zhqyy.ticketprovider、pmp.http.ticketprovider
...
|方法名
|微服务的方法名称
|小写字母+下划线+数字+斜杠，斜杠用于表示分层，如：common/get_user，order/submit，order/complete
...
* WF-SHA2：基于SHA2-256的签名验证。格式：Authorization: WF-SHA2+空格+access_id:sign
	...
	HY-Noise = a34f2b5e9077dd05
	post内容体 = {"test":"abc"}
	HY-Content-Sign = o08rXpB33QV3Qt4uoZnHMS30xSp1mXC88IzsrOEp+ck=
	HY-Tag = 无
	HY-Channel = 无
	字串拼接：
